...
def predict_image(image: Image.Image, model, labels: Dict[int, str]) -> Optional[PredictionResult]:
    ...
    if show_all and len(result.all_predictions) > 1:
        ...
        for label, conf in result.all_predictions:
            ...
            with col1:
                ...
                st.progress(conf)
            with col2:
                st.markdown(f"<p style='text-align: right; margin-top: 0.5rem;'>{conf * 100:.1f}%</p>", unsafe_allow_html=True)
...
def main():
    ...
    st.markdown("<p style='text-align: center; color: #666;'>Identify flowers in real-time or from uploaded images</p>", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if ctx.state.playing:
            ...
            if result:
                ...
                show_status_message("🔍 Point your camera at a flower to start detecting...", "waiting")
        else:
